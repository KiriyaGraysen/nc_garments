...
<head>
    ...
    <title>Account Recovery | NC Garments</title>
...
<body class="bg-gray-50 dark:bg-zinc-950 min-h-screen flex items-center justify-center p-4 antialiased font-sans transition-colors duration-300 relative overflow-hidden">
    ...
    <div class="w-full max-w-[420px] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-zinc-800 overflow-hidden relative z-10 mt-10">
        ...
        <div class="pt-10 pb-6 px-8 text-center relative">
            ...
            <p class="text-xs font-medium text-gray-500 dark:text-zinc-400 mt-3 leading-relaxed">
                Enter your registered email address. If it exists in our system, we will send you instructions to securely reset your password.
            </p>
        </div>

        <div class="px-8 pb-8">
            
            <div id="alert-container" class="mb-5 hidden"></div>

            <form id="recovery-form">
                <div class="mb-6">
                    <label for="recovery_id" class="block text-xs font-semibold text-gray-600 dark:text-zinc-400 mb-1.5 uppercase tracking-wide">Email Address</label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-zinc-500">
                            <i class="fa-solid fa-envelope text-sm"></i>
                        </span>
                        <input type="email" id="recovery_id" required placeholder="e.g., staff@ncgarments.com" 
    ...
    <script>
        ...
        recoveryForm.addEventListener('submit', async function(event) {
            ...
            try {
                ...
                if (data.status === 'success') {
                    // Show a persistent success message above the form
                    alertContainer.innerHTML = `
                        <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 rounded-xl p-3 text-center flex flex-col items-center justify-center gap-1">
                            <div class="flex items-center gap-2">
                                <i class="fa-solid fa-circle-check text-emerald-500"></i>
                                <h4 class="font-bold text-emerald-800 dark:text-emerald-400 text-sm">Request Processed</h4>
                            </div>
                            <p class="text-[11px] text-emerald-600 dark:text-emerald-500/80 font-medium">If an account matches that email, a reset link has been sent.</p>
                        </div>
                    `;
                    alertContainer.classList.remove('hidden');
                    
                    // Trigger the 60-second cooldown
                    startCooldown(60);
        ...
        function startCooldown(seconds) {
            ...
            recoverBtn.innerHTML = `Resend available in ${timeLeft}s`;
